...
function MyMangas() {
    ...
    const imageMangaNoFound = <img className="w-[10rem] right-80" src={mangaNoFound} alt="mangaNofound" />
    ...
    useEffect(
        () => {
            ...
            axios.get(apiUrl + `mangas/me?title=${title.current.value}&author_id=${userId}&category_id=${idArr.join(',')}&order=1&limit=6&page=${currentPage}`, headers)
                ...
                <div className="w-[80%] flex flex-col justify-between lg:min-h-[83vw] md:w-[80%] lg:w-[80%] lg:ml-10 mt-4">
                    ...
                    <div className="md:w-full lg:w-[90%] flex flex-wrap gap-2 md:gap-7 md:mr-2 w-full min-h-[16rem] md:min-h-[34rem] lg:min-h-[60vw] xl:min-h-[47vw]">
                        <Anchor to={`/manga-form`} key="-1" className="relative sm:h-[3rem] h-[8rem] md:h-[14rem] md:w-[12rem] lg:h-[19rem]">
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center duration-300 ease-in-out hover:text-[#ca2828] animate-pulse z-1 absolute">
                        ...
                        {mangas.length > 0 ? mangas.map((eachManga, index) => (
                            <Anchor to={`/manga/${eachManga._id}`} key={index} className="w-[22vw] md:w-[10rem] lg:w-[12rem] text-white md:h-[16rem] lg:h-[21rem] lg:min-h-[21rem]">
                                <div className="h-full flex-col flex justify-between">
                                    ...
                                    <div className=" flex absolute z-2">
                                        <button className="w-3 h-7 bg-[#1d1dc0] hover:bg-black "></button>
                                        <button className="w-3 h-7 bg-[#c25050] hover:bg-white "></button>
                                    </div>
                                    <p className="text-[2px] font-[200] cursor-pointer sm:font-[300] sm:text-sm md:text-md">{eachManga.title}</p>
                                </div>
                            </Anchor>
                        )) : <div className="flex relative items-center justify-center w-screen">
                            {imageMangaNoFound}
                            <p className="text-[#ffffff] absolute text-4xl font-black">¡Ups! No Mangas found</p>
                        </div>}
                    </div>
