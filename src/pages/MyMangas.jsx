...
function MyMangas() {
    ...
    const { read_mangas_me, delete_mangas_me } = mangas_actions
    ...
    const [id, setEditItemId] = useState(null)
    ...
    const editChapter = () => {
        navigate('/chapter-form/:id_manga')
    }
    const moreChapter = () => {
        navigate('/edit/:id_manga')
    }
    ...
    useEffect(
        () => {
            ...
            dispatch(read_mangas_me({ title: title.current.value || "", cates }))
    ...
    //<EditManga isOpen={isModalOpen} itemId={id} onClose={closeModalEdit} />
    return (
        <>
                ...
                <div className="w-[80%] flex flex-col justify-between  md:w-[80%] lg:w-[80%] lg:ml-10 mt-4">
                    ...
                    <div className="md:w-full lg:w-[90%] mb-16 flex flex-wrap gap-2 md:gap-7 md:mr-2 w-full min-h-[16rem] md:min-h-[34rem] lg:min-h-[0rem] xl:min-h-[47vw]">
                        ...
                        {mangas_me.length > 0 ? mangas_me.map((eachManga, index) => (
                            <div key={index} className="h-full w-[22vw] md:w-[10rem] text-[#ffffff90] z-10 lg:w-[12rem] text-white hover:text-white md:h-[16rem] lg:h-[21rem] lg:min-h-[21rem] flex-col flex justify-between hover:scale-[1.05] hover:-translate hover:drop-shadow-[0_0_100px_rgba(250,0,0,0.5)] duration-100 ease-in">
                                <img onClick={() => mangaDetails(eachManga._id)} className="sm:h-[3rem] h-[8rem] relative z-1 cursor-pointer md:h-[14rem] lg:h-[19rem] object-cover rounded-md" src={eachManga.cover_photo} alt="" />
                                <div className="w-[22vw] md:w-[10rem] lg:w-[12rem] md:h-[14rem] lg:h-[19rem] sm:h-[3rem] h-[8rem] flex flex-col items-center justify-between absolute z-2">
                                    <div className="w-full relative z-4 h-full flex justify-end p-3 text-[#ffffff] drop-shadow-[0_1px_1px_rgba(100,0,0,1)] hover:drop-shadow-[0_2px_3px_rgba(100,0,0,1)] duration-100">
                                        <BsFillPatchPlusFill onClick={moreChapter} className="hover:text-[#ffffff80] duration-100" />
                                        <RiEditBoxFill onClick={editChapter} className="hover:text-[#ffffff80] duration-100" />
                                    </div>
